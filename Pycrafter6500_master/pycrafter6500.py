...
import numpy
import sys
from Pycrafter6500_master import erle
...
class dmd():
    def __init__(self, backend):
        # self.dev = usb.core.find(idVendor=0x0451, idProduct=0xc900)  # todo
        # self.dev = usb.core.find(backend=backend, idVendor=0x0525, idProduct=0xa4a2)
        self.dev = usb.core.find(idVendor=0x0525, idProduct=0xa4a2)
    ...
    def defsequence(self, images, exp, ti, dt, to, rep):
        ...
        for i in range((num - 1) // 24 + 1):
            ...
            if i < ((num - 1) // 24):
                ...
                imagedata = arr[i * 24:]
            print('encoding...')
            imagedata, size = erle.encode(imagedata)  # todo encode to erle.encode
