...
class Camera:
    ...
    def measure(self):
        ...
        for i, mask in enumerate(self.hadamard_all[:20, ...]):  # todo first 20 only!
            ...
            # time.sleep(1/60)  # 16.666ms
            time.sleep(17 * 1e-3)  # 17ms
            ...
            time.sleep(self.measurement_time)  # todo measurement time

            # print(self.my_instrument.query('DATA: POINts?'))
            # if int(self.my_instrument.query('DATA: POINts?')) > 900:
            # buffer = self.multimeter.query('FETCh?').split(';')[0].split(',')
            # if len(buffer) > 950:  # todo 950? 1900?
            ...
            print(self.multimeter.query('FETCh?').split(';')[0].split(','))
